...
df<- cbind(astudies, data.frame(numcountry=v, countrycohort=v, hasHAZ=v, stuntprev=v, numsubj=v, numobs=v, numsubj_u24mo=v, numobs_u24mo=v, median_length_between_measures=v, 
                                firstage=v, lastage=v,
                                sd_obs=v, minages=v, maxages=v, mortality=v, birthweek=v, RCT=v, diar=v,
                                HAZsd=v, WAZsd=v, HAZsd=v,perc_length_decrease=v, 
                                ...
                                meanHAZ_m1=v, meanHAZ_m2=v, meanHAZ_m3=v, meanHAZ_m4=v, meanHAZ_m5=v, meanHAZ_m6=v, meanHAZ_m7=v, meanHAZ_m8=v,
...
for(i in 1:nrow(df)){
  ...
  meanHAZ_m1<- meanHAZ_m2<- meanHAZ_m3<- meanHAZ_m4<- meanHAZ_m5<- meanHAZ_m6<- meanHAZ_m7<- meanHAZ_m8<-NA
  ...
  if(!is.null(d)){
    if("HAZ" %in% colnames(d)){
      ...
      for(j in 0:23){
        assign(paste('meanHAZ_m', j+1, sep=''),  mean(d$HAZ[d$AGEDAYS>=j*30 & d$AGEDAYS<(j+1)*30], na.rm=T))
  ...
  res<-t(c(numcountry, countrycohort, hasHAZ, stuntprev, numsubj, numobs, numsubj_u24mo, numobs_u24mo, median_length_between_measures, firstage, lastage, sd_obs, minages, maxages, mortality, birthweek, RCT, diar, 
           ...
           meanHAZ_m1, meanHAZ_m2, meanHAZ_m3, meanHAZ_m4, meanHAZ_m5, meanHAZ_m6, meanHAZ_m7, meanHAZ_m8,
