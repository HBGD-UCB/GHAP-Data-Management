...
poolRR <- function(d, family="binomial"){
  ...
  if(family=="binomial"){
    ...
    if(d$intervention_level[1] == d$baseline_level[1]){
      est <- data.frame(logRR.psi=0, logSE=0, RR=1, RR.CI1=1, RR.CI2=1, Nstudies= nstudies$N, baseline=d$baseline_level[1])
    }else{
      ...
      est$baseline <- d$baseline_level[1]
    }
  }else{
    if(d$intervention_level[1] == d$baseline_level[1]){
      est <- data.frame(ATE=0, SE=0, CI1=0, CI2=0, Nstudies= nstudies$N)
    }else{
      ...
      est$CI1<-(est$ATE - 1.96 * est$SE)
      est$CI2<-(est$ATE + 1.96 * est$SE)
